    ...
    if (event.touches.length === 1) {
        ...
        if (touchedModel) {
            ...
            longPressTimeout = setTimeout(() => {
                if (touchMoveCount < 5) { // Only trigger long press if there's minimal movement
                    handleLongPress(touch);
                }
            }, 500);
            
            showTouchInstruction('Drag to rotate');
            event.preventDefault(); // Prevent default only when touching a model
        }
    }
}, { passive: false });
...
renderer.domElement.addEventListener('touchmove', (event) => {
    ...
    if (currentTime - lastTouchTime < 16) { // Limit to ~60fps
        return;
    }
    ...
    // Handle rotation with improved performance
    if (isRotating && touchedModel && !isLongPress) {
        const rotationDelta = (touch.clientX - lastTouchX) * (isMobile ? 0.005 : 0.01); // Slower rotation on mobile
...
const loadModel = (index, file) => {
    ...
    const baseUrl = window.location.hostname === 'thenetwork1ng.github.io' 
        ? '/Tshirtphone' // Updated repository name
        : '';
...
const adjustForScreenSize = () => {
    ...
    camera.position.z = isMobileView ? 6 : 4;
    
    // Adjust model scale and quality
    Object.values(models).forEach(model => {
        if (model) {
            const scale = isMobileView ? 1.5 : 2;
            ...
            if (isMobile) {
                model.traverse((child) => {
                    if (child.isMesh) {
                        ...
                        child.material.metalness *= 0.8; // Reduce metalness for better performance
                        child.material.roughness *= 1.2; // Increase roughness for better performance
                    }
                });
            }
        }
    ...
    if (isMobileView) {
        renderer.setPixelRatio(1); // Use lower pixel ratio on mobile
        scene.fog.far = 20; // Reduce fog distance on mobile
    } else {
...
function animate(currentTime) {
    ...
    // Update indicators with reduced frequency on mobile
    if (!isMobile || currentTime % 2 === 0) {
    ...
    if (isTransitioning) {
        transitionProgress += isMobile ? 0.02 : 0.016; // Slightly faster transitions on mobile
    ...
    // Update models with optimized animations
    Object.entries(models).forEach(([key, model]) => {
        if (model && !isDragging) {
            const index = parseInt(key.replace('model', ''));
            
            // Calculate base floating position with reduced frequency
            const time = currentTime * 0.001;
            const floatingY = 0.2 + Math.sin(time + index) * (isMobile ? 0.05 : 0.1);
            ...
            if ((hoveredModel === key && !isTransitioning) || (isLongPress && hoveredModel === key)) {
                targetZ = 1.5;
                targetSpotlightIntensity = isMobile ? 1.5 : 2; // Reduce intensity on mobile
            }
            ...
            // Apply position changes with optimized smoothing
            const positionSmoothing = isMobile ? 0.15 : 0.08;
            model.position.x += (targetX - model.position.x) * positionSmoothing;
            model.position.y += (targetY - model.position.y) * (isMobile ? 0.1 : 0.05);
            model.position.z += (targetZ - model.position.z) * positionSmoothing;
            
            // Update spotlight with reduced frequency on mobile
            if (!isMobile || currentTime % 2 === 0) {
            ...
            // Apply material changes with optimized frequency
            if (!isMobile || currentTime % 3 === 0) {
                model.traverse((child) => {
                    if (child.isMesh && child.material) {
                        ...
                        if (isMobile) {
                            child.material.envMapIntensity = hoveredModel === key ? 1.4 : 1.0;
                        } else {
